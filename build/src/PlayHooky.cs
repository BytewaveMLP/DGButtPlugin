	...
	public class HookManager {
		...
		public unsafe void Hook(MethodInfo original, MethodInfo replacement) {
			...
			if ((object) original == null)
				throw new ArgumentNullException("original"); //original function should not be null
			if ((object) replacement == null)
				throw new ArgumentNullException("replacement"); //replacement function should not be null
			if ((object) original == (object) replacement)
				throw new ArgumentException("A function can't hook itself"); //original and replacement cannot be the same function

			//Hook sanity checks.
			if (original.IsGenericMethod)
				throw new ArgumentException("Original method cannot be generic"); //original function can't be generic
			if (replacement.IsGenericMethod || !replacement.IsStatic)
				throw new ArgumentException("Hook method must be static and non-generic"); //hook method must not be static and non-generic
			if (hooks.ContainsKey(original))
				throw new ArgumentException("Attempting to hook an already hooked method"); //a method that is already hooked cannot be hooked again
		...
		public unsafe void Unhook(MethodInfo original) {
			...
			//Hook sanity checks.
			if (!hooks.ContainsKey(original))
				throw new ArgumentException("Method was never hooked"); //make sure we ever hooked the method
